...
**현재 상태: 점수 지급 일시 중지.** 게임을 늘리는 동안 게임마다 점수 스케일을
맞추는 게 번거로워서, 각 게임 `script.js`의 `POINTS_ENABLED = false`로 점수
지급만 꺼뒀습니다(로그인/랭킹 UI, 계정 데이터는 그대로 있음 — 기존에 쌓인
점수도 그대로 보임). 게임을 다 추가하고 나서 한 번에 점수 정책을 다시 잡을 때
게임별로 이 플래그만 `true`로 되돌리면 됩니다.

**점수 정책** (게임별 `script.js`에 정의, 재개 시 참고용):
