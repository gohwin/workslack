...
이 사이트의 게임 3개(스도쿠, 지뢰찾기, 크로스워드)는 모두 계정 없이 로컬에서
완전히 동작합니다. 계정을 만들면 **깰 때마다 매번** 점수가 쌓이고(하루 1회 같은
제한 없음), 그 점수가 메인 페이지 사이드바의 두 랭킹에 반영됩니다.
...
- 크로스워드: 완성할 때마다 고정 10점
- 스도쿠 / 지뢰찾기: 난이도별 차등 — 쉬움 5점, 중간 10점, 어려움 20점
...
service cloud.firestore {
  match /databases/{database}/documents {
    match /crossword-users/{uid} {
      ...
      // 점수 적립(1~20점 증가, 닉네임 불변) 또는 닉네임 변경(점수 불변)만
      // 허용 -- 둘 다 동시에 바뀌는 요청은 막는다. 1~20 범위는 현재 게임들의
      // 점수 정책(최대 20점)에 맞춘 값이라, 어떤 게임이든 한 번에 20점을
      // 넘는 점수를 주게 되면 이 숫자도 같이 올려야 한다.
      allow update: if request.auth != null && request.auth.uid == uid
                    && (
                      (request.resource.data.totalScore > resource.data.totalScore
                       && request.resource.data.totalScore <= resource.data.totalScore + 20
